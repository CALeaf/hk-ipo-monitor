...
### 实时字段手动输入（overrides.json）

超购倍数 / 基石 / A+H 折让这些动态数据在公开页面很难稳定抓到，但你自己看富途/捷利只要 10 秒。建议的工作流：

1. 启动 Monitor 后，TG 推来一只新股先给出只基于保荐人+市值+行业的初版评分
2. 你在富途/捷利看到超购倍数、基石名单后，编辑 `data/overrides.json`（从 `overrides.json.example` 复制一份）加上这只股票的字段
3. 下次 Monitor 自动 run 时检测到分数或建议变化 → 自动重发更新版 TG 消息

`overrides.json` 在 `.gitignore` 里，不会被 commit。
